...
const ReceiptModal = ({ isOpen, onClose, type, data }) => {
  ...
  const [buyerChargesPerBag, setBuyerChargesPerBag] = useState({ labour: 0, silai: 0, transport: 0 })

  // Predefined charges for Wheat
  const farmerCharges = { 50: 7.16, 30: 4.30 }
  const buyerCharges = {
    50: { labour: 7.50, silai: 1.71, transport: 3.15 },
    30: { labour: 4.46, silai: 1.71, transport: 2.29 }
  }
  ...
  useEffect(() => {
    ...
    else setBagWeight(50)
    if (data?.bags) setBags(data.bags.toString())
    else setBags('')
    if (data?.crop === 'Wheat') {
      const bw = data.bagWeight || 50
      setFarmerChargesPerBag(farmerCharges[bw] || 0)
      const buyer = buyerCharges[bw] || { labour: 0, silai: 0, transport: 0 }
      setBuyerChargesPerBag(buyer)
    } else {
      setFarmerChargesPerBag(0)
      setBuyerChargesPerBag({ labour: 0, silai: 0, transport: 0 })
    }
  }, [isOpen, data])
  ...
  let farmerTotal = 0
  let buyerLabourTotal = 0, buyerSilaiTotal = 0, buyerTransportTotal = 0
  let finalAmount = 0
  ...
  if (type === 'I') {
    ...
    buyerTransportTotal = buyerChargesPerBag.transport * numBags
    finalAmount = totalPrice - (buyerLabourTotal + buyerSilaiTotal + buyerTransportTotal) // For J‑Form we also deduct? Actually J‑Form is for buyer, we might show costs only. I'll keep as total price minus buyer costs for consistency.
  }

  const formatNumber = (num) => (isNaN(num) ? '0.00' : num.toFixed(2))

  if (!isOpen) return null
  if (!data || !data.farmerName) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
        <div className="bg-white p-6 rounded-2xl">Error: Receipt data incomplete.</div>
      </div>
    )
  }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-800 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        {/* Editable fields inside modal (optional) */}
            ...
            <tbody>
              ...
              {type === 'J' && (
                <>
                  ...
                  <tr><td className="p-2">Transport @ ₹{buyerChargesPerBag.transport}/bag</td><td className="p-2 text-right">{formatNumber(buyerTransportTotal)}</td></tr>
                  <tr className="font-bold bg-blue-50"><td className="p-2">TOTAL BUYER COSTS</td><td className="p-2 text-right text-primary">{formatNumber(buyerLabourTotal + buyerSilaiTotal + buyerTransportTotal)}</td></tr>
                </>
              )}
            </tbody>
          </table>
